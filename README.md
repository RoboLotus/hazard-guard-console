...
### 지도 세션별 설비 ROI와 순찰 경로

운용 순서는 `2D 지도 작성 → 지도 저장 → 저장 지도 기반 RGB-D 수집 → PLY 저장
→ 설비 ROI·웨이포인트 등록 → 순찰`입니다. 지도 탭은 활성 세션의 2D 지도와
`map` 좌표계 PLY가 모두 준비된 뒤에만 설비 등록을 허용합니다.

- 2D 지도에서 설비 중심을 추가하고 X/Y/Z 범위를 조정합니다.
- 3D RGB-D 화면에서 선택한 설비의 ROI 박스를 저장 PLY 위에 확인합니다.
- 활성 설비 ROI끼리 겹치거나 3 cm보다 가까우면 프런트엔드와 백엔드가 모두
  저장을 거부합니다.
- 설비와 웨이포인트는 브라우저가 아니라 Jetson의
  `runtime/maps/<world>/<session>/equipment.json`, `route.json`에 저장됩니다.
- 새 2D 지도 세션은 빈 설비·빈 경로로 시작합니다. 예전 목업/`odom` ROI는
  자동으로 승계하지 않습니다.
- 같은 지도 세션에서 PLY를 더 조밀하게 다시 만든 경우 좌표계가 그대로이므로
  ROI를 보존하고, 형상 fingerprint 변화만 재확인 정보로 사용합니다.
